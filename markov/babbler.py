...
class Babbler:
    ...
    def add_sentence(self, sentence):
        ...
        for word in words:
            word = word.lower()
        words.append("EOL")
        #Add ngrams into new list to make life easier
        for i in range(0, len(words) - self.n):
            k = ""
            for j in range(i, i + self.n):
                k += words[j] + " "
            k = k.rstrip()
            if k in self.mapOfWord.keys():
                self.mapOfWord[k].append(words[i + self.n])
            else:
                temp = [words[i + self.n], ]
                self.mapOfWord[k] = temp
            if k not in self.ngramList:
                self.ngramList.append(k)
            if i == 0:
                self.starterList.append(k)
    ...
    def get_stoppers(self):
        ...
        """
        stoppers = []
        for word in self.mapOfWord.keys():
            for w in self.mapOfWord[word]:
                if w == "EOL":
    ...
    def get_successors(self, ngram):
        ...
        """
        succ = []
        if ngram in self.mapOfWord:
            succ = list(self.mapOfWord[ngram])
        return succ
    ...
    def get_random_successor(self, ngram):
        ...
        """
        if ngram in self.mapOfWord:
            return random.choice(self.mapOfWord[ngram])
    ...
    def babble(self):
        ...
        """
        sen = random.choice(self.starterList)
        sen += " " + random.choice(self.mapOfWord[sen])
        tempArr = sen.split(" ")
        nextWord = ""
        for i in range(1, len(tempArr)):
            nextWord += tempArr[i] + " "
        nextWord = nextWord.rstrip()
        while True:
            nextWord = self.get_random_successor(nextWord)
            if nextWord == "EOL":
                break
            sen += " " + nextWord
            tired = sen.split(" ")
            nextWord = ""
            for i in range (len(tired) - self.n, len(tired)):
                nextWord += tired[i] + " " 
...
if __name__ == '__main__':
    ...
    filename = 'tests/books/shakespeare-caesar.txt'
    num_sentences = 5
    if len(sys.argv) > 0:
